...
class App extends Component {
  ...
  componentDidMount(){
    ...
    if (this.state.activeNodeId && this.state.graph.getNode(this.state.activeNodeId)){
  ...
  setActiveNodeId(id, addToHistory=true){
    ...
    var event = new CustomEvent("MIM_GRAPH_ACTIVE_NODE", { detail: {"nodeId": id }});
 ...
 // set active link id in the graph and update the state
  setActiveLinkId(id){
    ...
    if (id){
      ...
      if (this.state.graph.config.activeNodeId && this.state.graph.getNode(this.state.graph.config.activeNodeId)){
        let hit = false;
        let links = this.state.graph.getVisibleLinks();
        links.some((link) => { if (link.id === id) { hit = true; return true; } return false; })
        if (!hit){
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <CSSTransitionGroup className="panel-transition" transitionName="fade-wait"
                                   transitionEnterTimeout={400}
                                   transitionLeaveTimeout={200}>
                            {activeNode && this.state.panelActive ? <Panel key={activeNodeId} active={this.state.panelActive}
                             height={this.state.height}
                             node={activeNode}
        ...
        <div className="steps">
          <div title="Full graph view" className={["home icon-button",this.state.activeNodeId ? '' : 'inactive'].join(" ")} onClick={()=>{this.setActiveNodeId(null); }}>HOME</div>
          <div title={graph.hasPrev() ? "Go back" : "No more steps to go back"} className={["prev icon-button",graph.hasPrev() ? "" : "inactive"].join(" ")} onClick={this.historyPrev.bind(this)}>PREV</div>
          <div title={graph.hasNext() ? "Go forward" : "No more steps to go forward"} className={["next icon-button",graph.hasNext() ? "" : "inactive"].join(" ")} onClick={this.historyNext.bind(this)}>NEXT</div>
        </div>

        { !this.state.helpActive ? <div title="Show additional information about this visualization" onClick={() => { this.setHelpActive(true); }}  className="icon-button help-button">HELP</div> : null }

        { activeNode ? <div className={["toggle-panel icon-button",this.state.panelActive ? "close" : null].join(" ")} onClick={this.togglePanelActive.bind(this)}>{ this.state.panelActive ? "CLOSE" : "INFO" }</div> : null }

        <CSSTransitionGroup className="help-transition" transitionName="fade-wait"
                                   transitionEnterTimeout={400}
                                   transitionLeaveTimeout={200}>
        { this.state.helpActive ? <Help height={this.state.height} /> : null}
        </CSSTransitionGroup>

        { this.state.helpActive ? <div className="icon-button close-help-button" onClick={() => { this.setHelpActive(false); }} >CLOSE</div> : null }
      </div>
    );
  }
}
